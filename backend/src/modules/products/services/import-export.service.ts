...
export class ProductImportExportService {
  ...
  async importProducts(
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const salePrice = row.preco || row.price || row.preco_venda || row.salePrice;
        const sku = row.sku || this.generateSku();

        // Validar campos obrigatórios
        const validationErrors: string[] = [];
        if (!name) validationErrors.push('Nome é obrigatório');
        if (salePrice === undefined || salePrice === null) validationErrors.push('Preço de venda é obrigatório');
        ...
        const importNotes = needsReview.length > 0 ? needsReview.join('; ') : null;
        ...
        const productData = {
          ...
          promoPrice: row.preco_promo || row.promoPrice ? Number(row.preco_promo || row.promoPrice) : null,
  ...
  async exportProducts(
    tenantId: string,
    options: { categoryId?: string; status?: string; includeVariations?: boolean; format?: 'xlsx' | 'csv' } = {},
  ): Promise<Buffer> {
  ...
  async getImportTemplate(): Promise<Buffer> {
    ...
    const templateData = [
      ['sku', 'nome', 'descricao', 'categoria', 'fornecedor', 'codigo_fornecedor', 'marca', 'cor', 'tamanho', 'material', 'origem', 'ncm', 'preco_custo', 'preco_venda', 'preco_promo', 'estoque', 'estoque_minimo', 'unidade', 'codigo_barras', 'ativo'],
      ['PROD001', 'Camiseta Básica Azul', 'Camiseta 100% algodão', 'Vestuário', 'Fornecedor ABC', 'FOR001', 'Marca X', 'Azul', 'M', 'Algodão', 'Nacional', '61091000', 50.00, 99.90, 79.90, 100, 10, 'UN', '7891234567890', 'Sim'],
      ['PROD002', 'Calça Jeans', 'Calça jeans tradicional', 'Vestuário', 'Fornecedor XYZ', 'FOR002', 'Marca Y', 'Azul Escuro', '42', 'Jeans', 'Nacional', '62034200', 80.00, 159.90, '', 50, 5, 'UN', '7891234567891', 'Sim'],
      ['PROD003', 'Tênis Esportivo', '', '', 'Fornecedor ABC', '', '', 'Preto', '40', '', 'Importado', '', 120.00, 249.90, 199.90, 30, 5, 'PAR', '', 'Sim'],
      ['', 'Produto sem SKU (será gerado)', '', '', '', '', '', '', '', '', '', '', '', 49.90, '', 10, 5, 'UN', '', 'Sim'],
    ];
    ...
    const instructionsData = [
      ...
      ['sku', 'Não*', 'Código único do produto. Se não informado, será gerado automaticamente', 'PROD001'],
      ['nome', 'SIM', 'Nome do produto', 'Camiseta Básica Azul'],
      ['descricao', 'Não', 'Descrição detalhada do produto', 'Camiseta 100% algodão'],
      ['categoria', 'Recomendado', 'Nome da categoria (deve existir no sistema)', 'Vestuário'],
  ...
  async linkVariations(tenantId: string, parentProductId: string, variationIds: string[]): Promise<any> {
  ...
  async unlinkVariations(tenantId: string, variationIds: string[]): Promise<any> {
    const updated = await this.prisma.product.updateMany({
      where: {
        id: { in: variationIds },
        tenantId,
