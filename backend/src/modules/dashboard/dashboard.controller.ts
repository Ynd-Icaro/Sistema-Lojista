...
export class DashboardController {
  ...
  getTopProducts(@CurrentUser() user: any, @Query("limit") limit?: string) {
  ...
  @ApiQuery({ name: "limit", required: false, type: Number })
  getRecentSales(@CurrentUser() user: any, @Query("limit") limit?: string) {
  ...
  getPendingServiceOrders(
    ...
    return this.dashboardService.getPendingServiceOrders(
      user.tenantId,
      limit ? parseInt(limit) : 5,
    );
  }
