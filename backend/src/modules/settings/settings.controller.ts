...
export class SettingsController {
  ...
  async getUserPermissions(@Request() req) {
    return this.settingsService.getUserPermissions(req.user.tenantId, req.user.role);
  }
  ...
  async updateNotifications(@Request() req, @Body() data: UpdateNotificationsDto) {
  ...
  async updatePermissions(@Request() req, @Body() data: UpdatePermissionsDto) {
    return this.settingsService.updatePermissions(req.user.tenantId, req.user.role, data);
  }

  @Post('permissions/reset')
  @ApiOperation({ summary: 'Reset permissions to default (Admin only)' })
  async resetPermissions(@Request() req) {
    return this.settingsService.resetPermissionsToDefault(req.user.tenantId, req.user.role);
  }
  ...
  async updateGeneralSettings(@Request() req, @Body() data: GeneralSettingsDto) {
    return this.settingsService.updateGeneralSettings(req.user.tenantId, req.user.role, data);
  }
  ...
  async updateViewProfiles(@Request() req, @Body() data: UpdateViewProfilesDto) {
    return this.settingsService.updateViewProfiles(req.user.tenantId, req.user.role, data);
  }
  ...
  async getUserIndividualPermissions(@Request() req, @Param('userId') userId: string) {
    return this.settingsService.getUserIndividualPermissions(req.user.tenantId, userId);
  }

  @Patch('user-permissions')
  @ApiOperation({ summary: 'Update individual user permissions' })
  async updateUserIndividualPermissions(@Request() req, @Body() data: UpdateUserPermissionsDto) {
    return this.settingsService.updateUserIndividualPermissions(req.user.tenantId, req.user.role, data);
  }

  @Post('user-permissions/:userId/reset')
  @ApiOperation({ summary: 'Reset user permissions to default for their role' })
  async resetUserPermissionsToDefault(@Request() req, @Param('userId') userId: string) {
    return this.settingsService.resetUserPermissionsToDefault(req.user.tenantId, req.user.role, userId);
  }

  @Get('user-permissions/:userId/effective')
  @ApiOperation({ summary: 'Get effective user permissions (combining role and individual)' })
  async getEffectiveUserPermissions(@Request() req, @Param('userId') userId: string) {
    return this.settingsService.getEffectiveUserPermissions(req.user.tenantId, userId);
  }

  @Get('my-permissions')
  @ApiOperation({ summary: 'Get current logged in user effective permissions' })
  async getMyEffectivePermissions(@Request() req) {
    return this.settingsService.getEffectiveUserPermissions(req.user.tenantId, req.user.userId);
  }
}
