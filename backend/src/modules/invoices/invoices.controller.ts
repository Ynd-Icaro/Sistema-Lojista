...
export class InvoicesController {
  ...
  @ApiOperation({
    summary: "Buscar nota fiscal pela chave de acesso (público)",
  })
  ...
  async getHtmlPublic(
    ...
    const html = await this.invoicesService.getInvoiceHtml(
      invoice.id,
      invoice.tenantId,
    );
    return res.send(html);
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Gerar nota fiscal a partir de venda ou ordem de serviço",
  })
  generate(
    @CurrentUser("tenantId") tenantId: string,
