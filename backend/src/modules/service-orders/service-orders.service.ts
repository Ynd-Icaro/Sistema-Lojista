...
export class ServiceOrdersService {
  ...
  async create(tenantId: string, userId: string, dto: CreateServiceOrderDto) {
    ...
    if (order.customer) {
      try {
        await this.notificationsService.sendServiceOrderUpdate(tenantId, {
          ...order,
          code: order.code,
        }, 'CREATED');
      } catch (error) {
        this.logger.error(`Erro ao enviar notificação de OS criada: ${error.message}`);
      }
    }
  ...
  async updateStatus(id: string, tenantId: string, status: string, notes?: string) {
    ...
    if (status === 'COMPLETED' && previousStatus !== 'COMPLETED') {
      try {
        ...
        if (Number(updatedOrder.total) > 0) {
          ...
          this.logger.log(`Transação financeira criada: OS #${updatedOrder.code} - R$ ${updatedOrder.total}`);
        }

        // Gerar Nota Fiscal de Serviço
        const invoice = await this.invoicesService.generateFromSource(tenantId, {
          type: 'SERVICE',
          serviceOrderId: updatedOrder.id,
          warrantyDays: updatedOrder.warrantyDays || 90,
          notes: `Referente à Ordem de Serviço #${updatedOrder.code}`,
        });
        this.logger.log(`NF de serviço gerada: ${invoice.number} para OS #${updatedOrder.code}`);
      } catch (error) {
        this.logger.error(`Erro ao criar transação/NF para OS #${updatedOrder.code}: ${error.message}`);
      }
    }
    ...
    if (status === 'DELIVERED' && previousStatus !== 'DELIVERED') {
      try {
        ...
        if (invoice && updatedOrder.customer?.email) {
          await this.invoicesService.sendToCustomer(invoice.id, tenantId, ['email']);
          this.logger.log(`NF ${invoice.number} enviada por email para ${updatedOrder.customer.email}`);
        }
      } catch (error) {
        this.logger.error(`Erro ao enviar NF por email para OS #${updatedOrder.code}: ${error.message}`);
      }
    }
    ...
    if (updatedOrder.customer && status !== previousStatus) {
      try {
        ...
        this.logger.log(`Notificação enviada: OS #${updatedOrder.code} - ${previousStatus} → ${status}`);
      } catch (error) {
        this.logger.error(`Erro ao enviar notificação de mudança de status: ${error.message}`);
      }
    }
  ...
  async remove(id: string, tenantId: string) {
    ...
    if (order.status !== 'PENDING' && order.status !== 'CANCELLED') {
      throw new BadRequestException('Só é possível remover OS pendentes ou canceladas');
    }
  ...
  async getStats(tenantId: string) {
    const [pending, inProgress, completed, delivered] = await Promise.all([
      this.prisma.serviceOrder.count({ where: { tenantId, status: 'PENDING' } }),
      this.prisma.serviceOrder.count({ where: { tenantId, status: 'IN_PROGRESS' } }),
      this.prisma.serviceOrder.count({ where: { tenantId, status: 'COMPLETED' } }),
      this.prisma.serviceOrder.count({ where: { tenantId, status: 'DELIVERED' } }),
    ]);
